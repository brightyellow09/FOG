...
from statistic.policy import TabularPolicy
...
def _regret_matching(cumulative_regrets, legal_actions):
    ...
    if sum_positive_regrets > 0:
        for i in range(len(legal_actions)):
            ...
            info_state_policy[i] = positive_action_regret / sum_positive_regrets
    else:
        for i in range(len(legal_actions)):
...
class CFR(Solver):
    ...
    def __init__(self, args):
        ...
        self._game = init_env(args['env'])
        self._num_players = self._game.num_players()
        self._root_node = History(self._game) #!!!
    ...
    def _compute_counterfactual_regret_for_player(self, history, reach_probabilities, player):
        ...
        if history[-1].next_state.is_chance():
            history_value = 0.0
            for action, action_prob in history[-1].next_state.chance_outcomes():
        ...
        info_state_policy = self._current_policy.action_probability_table[
        ...
        for i in range(len(legal_actions)):
            cfr_regret = counterfactual_reach_prob * (
                children_utilities[i][current_player] - history_value_for_player
            )
            info_state_node.cumulative_regret[i] += cfr_regret
            info_state_node.cumulative_policy[i] += reach_prob * info_state_policy[i]
        return history_value
    ...
    def evaluate_and_update_policy(self):
        for player in range(self._num_players):
            self._compute_counterfactual_regret_for_player(
                self._root_node,
                reach_probabilities = np.onees(self._num_players+1),
                player = player
            )
